...
export const useStore = create(
  persist(
    (set, get) => ({
      ...
      setUser: async (user) => {
        const { syncCartFromLocalStorage, loadCartFromSupabase } = get();
      ...
      addToCart: async (book, qty = 1) => {
        ...
        const { user } = get();
        if (user?.id) await get().syncCartToSupabase(newCart);
      },
      ...
      addToWishlist: (book) => {
        ...
        const { wishlist } = get();
        if (!wishlist.some((i) => i.book_id === book.book_id)) {
      ...
      removeFromWishlist: (book_id) => {
        const { wishlist } = get();
        set({ wishlist: wishlist.filter((i) => i.book_id !== book_id) });
      },
